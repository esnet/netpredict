...
class NetPredict():
    ...
    def __init__(self,batch_size=50,num_layers=2,window_length=144,save_model="stgcnwavemodel.pt",
                 pred_len=48,channels=[1, 16, 32, 64, 32, 128],control_str='TNTSTNTST',output_dir='log_training',device=None,**kwargs):
        ...
        device = torch.device(device) if device!=None else torch.device("cuda") if torch.cuda.is_available() else torch.device("cpu")
        ...
        self.test = scaler.transform(test)
        ...
        x_test, y_test = data_transform(self.test, window_length, pred_len, device)

        train_data = torch.utils.data.TensorDataset(x_train, y_train)
        self.train_iter = torch.utils.data.DataLoader(train_data, batch_size, shuffle=True)
        val_data = torch.utils.data.TensorDataset(x_val, y_val)
        self.val_iter = torch.utils.data.DataLoader(val_data, batch_size)
        test_data = torch.utils.data.TensorDataset(x_test, y_test)
        self.test_iter = torch.utils.data.DataLoader(test_data, batch_size)

        G = G.to(device)
        self.model = STGCN_WAVE(channels, window_length, n_route, G, drop_prob, num_layers, device, control_str).to(device)
        self.best_model = copy.deepcopy(self.model)

    def train_model(self,lr=0.001,epochs=50,**kwargs):
        ...
        for epoch in range(1, epochs + 1):
            ...
            for i,(x, y) in enumerate(self.train_iter):
                ...
                n += y.shape[0]
            scheduler.step()
            val_loss = evaluate_model(self.model, loss, self.val_iter)
            if val_loss < min_val_loss:
                min_val_loss = val_loss
                torch.save(self.model.state_dict(), '%s/%s' % (self.output_dir,self.save_model))
            train_loss = l_sum / n
        ...
        self.best_model.load_state_dict(torch.load('%s/%s' % (self.output_dir,self.save_model)))

        self.loss = evaluate_model(self.best_model, loss, self.test_iter)
        MAE, MAPE, RMSE = evaluate_metric(self.best_model, self.test_iter, scaler)

        with open("%s/results.txt" % self.output_dir,"w") as f:
    ...
    def predict(self,path_to_model,**kwargs):
        self.model.load_state_dict(torch.load(path_to_model))
        pred_data = get_predict(self.model,self.test_iter)
        return self.test, pred_data

        
def main(**kwargs):
    net_predict = NetPredict(**kwargs)
    net_predict.train_model(**kwargs)
    return net_predict.loss
